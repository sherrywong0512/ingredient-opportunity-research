...
2. **一句话重复对照证据：** 三个预注册合成案例中，用户只提供一句决策请求；每种条件运行三次。按作者记录的分组，Skill 的 9 份输出均分 95.0、最低 87、总体标准差 4.7，Direct 均分 85.7、最低 60、总体标准差 11.4；两组均无硬失败且 headline decision 相同。输出在评分前提交，但分组映射未预提交或做哈希承诺，因此不能由第三方独立排除评分后重分组；精确模型版本、采样参数和逐字 harness 指令也未保存，因此只能复算已保存输出，不能独立复现生成。该结果是本 fixture set 中框架覆盖与重复运行稳定性差异的初步证据，不支持“行业认可”、真实市场准确率或一般模型优势；
3. **结构化提示校准证据：** 按作者记录的评分后分组，在同三类案例中，Direct 组总分为 283/300，Skill 组为 298/300，描述性平均差为 +5.0 分；两组均无硬失败。该分组同样未预提交或做哈希承诺，结果只用于初步校准审计完整性和行动可执行性；
4. **用户提供的初步专家试用反馈：** 专家认为系统有效、覆盖较全面，试用中未发现明显错误；相较原有人工搜索，估计节省约 80%–90% 时间，并因扩大信息覆盖而帮助提高判断准确性。

第四类反馈支持产品已经进入可用和持续验证阶段，但当前仓库尚未保存试用专家数量、原料清单、任务范围、人工基线用时、Skill 用时、错误定义和逐项评审记录。因此，80%–90% 和“提高准确性”应作为**用户报告的初步结果**，不能暂时写成已完成的独立量化验证。合成评分也不能替代这些真实工作流证据。
...
4. **盲评对照：** 同一输入、工具、时限和模型族，对比无 Skill 与 Skill；至少三次独立运行，报告最低分、极差和标准差，而不只报告均分；
