...
| ACES 式增量评估 | 配对 with-skill/baseline 试验下，Skill 是否带来边际价值（按 expected_behavior 逐条判分） | [6 案例（3 冻结复用 + 2 新 + 1 负例）、22 份输出盲评、哈希承诺分组密钥](../evaluation/aces/README.md) | 复合 lift ≈ 0.008、outcome-only ≈ −0.007（近零）；行为级增益仅在 adverse 发现与替代空白分类；路由 6/6 与 3/3 | 真实市场准确率、行业专家评审、跨模型泛化或业务结果 |
...
### ACES 式增量评估：配对差分与行为级判分

按 arXiv:2608.20614（ACES）的方法，把评测从"单一百分制均分对比"升级为"逐条 expected_behavior 判分 + 配对 Skill Lift"。结果诚实地说：**在这个语料上 Skill 的边际价值接近零**——复合 lift 0.0076（95% CI 含 0）、outcome-only −0.0067；行为级增益只出现在两处 Direct 组系统性漏掉的分类：adverse/null 发现分类（case-02：Skill 3/3 vs Direct 0/3）与替代空白（substitution whitespace）分类（case-04：Skill 1/1 vs Direct 0/1）。同时静态 rubric 总分与行为通过率相关性弱（Spearman 0.34）、与 accuracy 几乎无关（0.11），复现了论文"扫描分数不等于运行时证据"的论断——rubric 总分应读作报告结构质量，不是行为表现的代理。负例（非消费品边界）两臂都正确处理；路由测试 recall 6/6、precision 3/3、群组路由 3/3 无过度触发。与以往基准的区别：①分组映射在评审前用 SHA-256 哈希承诺并提交，弥补旧基准"未预承诺"的审计缺口；②新案例每条件仅 1 次运行，且仍是同模型族生成与评审，因此这仍是工作流证据，不是因果证明。
