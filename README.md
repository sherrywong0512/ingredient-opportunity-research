...
| 当前验证状态 | v1 初步开发完成；3 个预注册合成案例完成 Direct vs Skill 盲评，已有专家试用正向反馈，仍需真实案例验证准确性和市场匹配 |
...
Direct 组已经能做出安全的 headline decision；Skill 的优势主要体现在资本决策审计覆盖、不可比证据处理和下一步验证门槛，而不是把错误结论变成正确结论。协议、冻结输入、匿名原始输出、盲评分、组别揭盲和 CSV 数据均保存在仓库中。三个案例是针对已知失败模式设计的合成回归测试，不是无偏市场 benchmark。
...
│   └── three-case-comparison/              # 三案例协议、原始输出、盲评和 CSV 数据
